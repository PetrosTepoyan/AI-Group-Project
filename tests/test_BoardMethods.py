...
class TestBoardMethods(unittest.TestCase):
    ...
    def test_identical_boards_equality(self):
        board1 = Board(self.fence_checker, MoveChecker())
        board2 = Board(self.fence_checker, MoveChecker())
    ...
    def test_equality_same_fence_configurations(self):
        fences = {(1, 2), (2, 3)}
        board1 = Board(self.fence_checker, MoveChecker(), fences_horizontal=fences)
        board2 = Board(self.fence_checker, MoveChecker(), fences_horizontal=fences)
        self.assertEqual(board1, board2)

    def test_inequality_different_horizontal_fences(self):
        board1 = Board(self.fence_checker, MoveChecker(), fences_horizontal={(1, 2)})
        board2 = Board(self.fence_checker, MoveChecker(), fences_horizontal={(2, 3)})
        self.assertNotEqual(board1, board2)

    def test_inequality_different_vertical_fences(self):
        board1 = Board(self.fence_checker, MoveChecker(), fences_vertical={(1, 2)})
        board2 = Board(self.fence_checker, MoveChecker(), fences_vertical={(2, 3)})
        self.assertNotEqual(board1, board2)
